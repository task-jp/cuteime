...
#include "cuteimewidget.h"
#include <QPainter>
#include <QPainterPath>
#include <QTimer>
#include <math.h>

class CuteimeWidget::Private : private QObject
{
    Q_OBJECT
public:
    Private(CuteimeWidget *parent);
...
CuteimeWidget::Private::Private(CuteimeWidget *parent)
    : QObject(parent)
    , q(parent)
    , angle(0)
{
...
void CuteimeWidget::Private::update()
{
    angle = (angle + 5) % 360;
    if (q->isVisible()) {
        q->update();
...
void CuteimeWidget::Private::paint(QPainter &painter)
{
...
CuteimeWidget::CuteimeWidget(QWidget *parent)
    : QWidget(parent)
{
    d = new Private(this);
}

CuteimeWidget::~CuteimeWidget()
{
    delete d;
}

QSize CuteimeWidget::sizeHint() const
{
    return QSize(128, 128);
}

void CuteimeWidget::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
    QPainter p(this);
    d->paint(p);
