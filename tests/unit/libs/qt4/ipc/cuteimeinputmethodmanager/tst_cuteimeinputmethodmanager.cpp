...
class CuteimeInputMethodManagerTest : public QObject
{
    Q_OBJECT
public:
    CuteimeInputMethodManagerTest(QObject *parent = 0)
        : QObject(parent)
    {
        bool ret = QDBusConnection::sessionBus().registerService(CUTEIME_DBUS_SERVICE);
        if (ret) {
            inputmethodManager = new CuteimeInputMethodManager(this, CuteimeInputMethodManager::Server);
            inputmethodManager->init();
            QStringList args = QCoreApplication::arguments();
            args.removeFirst();
            client = QtConcurrent::run(QProcess::execute, QCoreApplication::applicationFilePath(), args);
        } else {
            inputmethodManager = new CuteimeInputMethodManager(this, CuteimeInputMethodManager::Client);
            inputmethodManager->init();
        }
    }

    ~CuteimeInputMethodManagerTest() {
        if (client.isStarted()) {
...
public slots:
    ...
    void init() {
        ...
        case CuteimeInputMethodManager::Server:
            break;
        case CuteimeInputMethodManager::Client:
            break;
        }
        expected.clear();
...
QTEST_MAIN(CuteimeInputMethodManagerTest)
#include "tst_cuteimeinputmethodmanager.moc"
